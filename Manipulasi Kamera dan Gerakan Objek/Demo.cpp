...
void Demo::ProcessInput(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
		...
		app.Start("Transformation: Transform Cube", 800, 600, false, false, 15);
	}
	...
	if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
		glfwSetWindowShouldClose(window, true);
		RenderEngine& app = Demo();
		app.Start("Transformation: Transform Cube", 800, 600, false, false, 4);
	}
...
void Demo::Update(double deltaTime) {
	angle += (float)((deltaTime * 0.8f) / 1000);
}
...
void Demo::Render(int x) {
	...
	// LookAt camera (position, target/direction, up)
	glm::mat4 view = glm::lookAt(glm::vec3(1, x, 12), glm::vec3(0, 1, 0), glm::vec3(0, 1, 0));
	GLint viewLoc = glGetUniformLocation(this->shaderProgram, "view");
	glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view));
